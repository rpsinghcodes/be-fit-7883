...
import { Loader2, Wand2, Sparkles, AlertCircle, Clipboard, Send, Eraser, ArrowRight, FileText, Tags, Maximize, BookOpen, MessageSquare, Languages, GitBranch, BotMessageSquare, Mic, Calculator } from 'lucide-react';
import { getAISuggestion, getContentImprovements, generateTitleSuggestion, summarizeNote, suggestTags, expandNote, getContentRecommendations, analyzeSentiment, translateText, generateMindMap, calculateAndCreateNote } from '../../components/services/aiService';
...
export default function AIWritingAssistant({ onInsert, currentContent, onUpdateTitle = () => {} }) {
  ...
  const actions = {
    ...
    calculate: { fn: calculateAndCreateNote, arg: prompt, label: 'Calculate', icon: Calculator, needsPrompt: true },
  };
                ...
                <CardContent className="p-4">
                  <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="grid w-full grid-cols-3 mb-4">
                      ...
                      <TabsTrigger value="calculate" className="text-gray-300">Calculate</TabsTrigger>
                    </TabsList>
                        ...
                        <TabsContent value="enhance" className="space-y-3">
                          {!isContentAvailable ? (
                            <Alert variant="warning" className="bg-yellow-800 text-yellow-100 border-yellow-600">
                              ...
                              <AlertDescription>
                                Ask some good foods suggestions.
                              </AlertDescription>
                            </Alert>
                          ) : (
                        ...
                        <TabsContent value="calculate" className="space-y-4">
                          <div className="relative">
                            <Textarea
                              value={prompt}
                              onChange={(e) => setPrompt(e.target.value)}
                              placeholder="Enter a calculation prompt (e.g., 'Calculate the area of a circle with radius 5 cm')"
                              className="bg-gray-700 text-white min-h-[100px] resize-y pr-10"
                            />
                            <Button
                              onClick={toggleSpeechToText}
